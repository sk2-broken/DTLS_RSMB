...
int Socket_addServerSocket(Listener* list)
{
	...
	else if (list->protocol == PROTOCOL_MQTTS_DTLS)
		{
			if (list->ipv6)
				list->socket = socket(AF_INET6, SOCK_DGRAM, 0);
			if (list->socket < 0 && ipv4)
	...
	if (list->protocol == PROTOCOL_MQTTS_DTLS)
	{
		...
		*pnewSd = list->socket;
		...
		/*if (setsockopt(list->socket, IPPROTO_IP, IP_MULTICAST_LOOP, (const char*)&loopback, sizeof(loopback)) == SOCKET_ERROR)
			Socket_error("set listener IP_MULTICAST_LOOP", list->socket);*/
		if (setsockopt(list->socket, IPPROTO_IPV6, IPV6_PKTINFO, (const char*)&loopback, sizeof(loopback)) == SOCKET_ERROR)
					Socket_error("set listener IP_MULTICAST_LOOP", list->socket);

		//TODO LW add dtls_new_context()
		printf("dtls_new_context()\n");
		list->dtls_context = dtls_new_context(&(list->socket));
		printf("dtls_set_handler()\n");
		dtls_set_handler(list->dtls_context, &cb);
		
		int i;
		printf("\ncookie :");
		for(i=0;i<12;i++)
		{
			printf("%x",(int)list->dtls_context->cookie_secret[i]);
		}
		printf("\n");

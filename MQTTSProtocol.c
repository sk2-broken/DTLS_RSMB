...
void MQTTSProtocol_timeslice(int sock)
{
	...
	FUNC_ENTRY;
	// If DTLS handshake is running pack = NULL
	// Else if DTLS handshake is finished pack must contained the MQTT-SN packet
	...
	if (clientAddr)
	{
		...
		if (pack == NULL)
		{
			...
			else if (error==DTLS_WAIT_FINISHED)	//  DTLS 'error' to notify RSMB state machine that the packet isn't malformed but is a DTLS handshake packet
			{
				printf("\nretransmission\n");
			}
		}
...
int MQTTSProtocol_handleConnects(void* pack, int sock, char* clientAddr, Clients* client)
{
	...
	else
	{
		...
		{
			client->connected = 1;
			printf("connack 2 begin\n");
			rc = MQTTSPacket_send_connack(client,0); /* send response */
			printf("connack 2 end\n");
		}
	}

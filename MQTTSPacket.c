 ...
 *	  Lucas Wagner 					- integration of DTLS 1.2  
 *******************************************************************************/
...
Listener* list;
	...
	n = recvfrom(list->socket, msg, max_packet_size, 0, from, &len);
	if (n == SOCKET_ERROR)
	{
		int en = Socket_error("UDP read error", list->socket);
		if (en == EINVAL)
	...
	*error = SOCKET_ERROR;  // indicate whether an error occurred, or not
	...
	{
		printf("\n\nReceive MQTTS packet\n");
		*clientAddr = Socket_getaddrname(from, list->socket);
	}
	else if((list->protocol)==PROTOCOL_MQTTS_DTLS) // If the protocol in use for this listener is mqtts_dtls
	{
		session_t session;
		dtls_peer_t* peer;

		memset(&session, 0, sizeof(session_t));		// clean DTLS session
		printf("\n\nReceive MQTTS DTLS packet\n");

		//	Copy the connection parameters into session struct
		session.size = len;
		memcpy(&session.addr.sa, from, len);

		//	Handle incoming DTLS messages  
		dtls_handle_message(list->dtls_context,&session,(uint8*)msg,n);
		
		//dtls_retransmit_process();
		*clientAddr = Socket_getaddrname((struct sockaddr *)&session.addr.sin6, list->socket);
		peer = dtls_get_peer(list->dtls_context, &session);

		
		if(peer)	// if peer exists
		{
			...
			{
				printf("\nSecured\n");
				// For application data dtls_handle_message() call read_from_peer()
				// read_from_peer copy the decrypted MQTT-SN packet into the dtls_context struct
				// Then we copy the decrypted MQTT-SN packet from this struct to msg used by RSMB
				memcpy(msg,&list->dtls_context->readbuf,list->dtls_context->decrypted_length);
				printf("length : %d\n",list->dtls_context->decrypted_length);
				n = list->dtls_context->decrypted_length;
				*error = DTLS_FINISHED;			// No error DTLS handshake finished, incoming packet are application data 
			}
		}
	}
...
exit:
	//TODO LW change dtls_free_context() location
	if ((list->protocol)==PROTOCOL_MQTTS)
	{	
		if(*error==SOCKET_ERROR)
		{
			dtls_free_context(list->dtls_context);
			printf("free\n");
		}
	}
   		FUNC_EXIT_RC(*error);
...
int MQTTSPacket_sendPacketBuffer(int socket, char* addr, PacketBuffer buf)
{
	...
	*(port - 1) = '\0';
	...
	if (strchr(addr, ':'))
	{
		...
		//TODO LW send encrypted packet
		Listener* list;
		dtls_peer_t *peer = NULL;
		list = Socket_getParentListener(socket);
		if(list->protocol==PROTOCOL_MQTTS_DTLS)
		{
			session_t session;
			socklen_t len = sizeof(struct sockaddr_in6);
			...
			session.size = len;
			memcpy(&session.addr.sin6,&cliaddr6, len);
			memcpy(&session.addr.sa, (struct sockaddr*) &session.addr.sin6, len);	
			...
			{
				printf("Encrypt\n");
				rc = dtls_write(list->dtls_context,&session,(uint8*)buf.data,buf.len);
			}
			else
			{
...
int MQTTSPacket_send(int socket, char* addr, MQTTSHeader header, char* buffer, int buflen)
{
	...
	*ptr++ = header.type;
	...
	if((Socket_getParentListener(socket)->protocol)==PROTOCOL_MQTTS_DTLS)
	{
		printf("\n\nSend MQTTS DTLS packet\n");
	}
	else if ((Socket_getParentListener(socket)->protocol)==PROTOCOL_MQTTS)
	{
		printf("\n\nSend MQTTS packet\n");
	}
